...
class Vk_api_access():
    ...
    def get_photos(self, user_id, photo_numb=3):
        photo_album = []
        photo_links = []
        photo = self.tools.get_all_slow('photos.get',
            100,
            {'owner_id': user_id,
             'album_id': 'profile',
             'extended': 1, })
        ...
        photo_album = sorted(photo_album, 
                             key = operator.itemgetter('photo_likes'), reverse = True)
    ...
    def search_friends(self, user_info):
        ...
        if user_info['gender'] == 1:
            ...
            sex = 0
        friends = self.tools.get_all('users.search', 
            100,
        ...
        result = [{'user_id': friend['id'],
                  ...
                  'user_link': f"https://vk.com/id{friend['id']}"} for 
                 friend in friends['items'] if 
